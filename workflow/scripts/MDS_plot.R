analysis=function(input, output, log) {
    ...
    colour=rainbow(length(unique(x$samples$group)))

    png(output$plot[1], width=2500, height=1800, res=400)
    par(mar=c(5,5,3,7), xpd=TRUE)
    plotMDS(x, labels=x$samples$group, col=colour,cex=0.8, main="MDS Plot")
        legend("topright", inset=c(-0.35,0), legend=c(unique(x$samples$group)), 
        col=colour, pch=15, box.lwd = 0,box.col = "white",bg = "white")
    dev.off()
    ...
    par(mar=c(5,5,3,7), xpd=TRUE)
    plotMDS(mat, labels=x$samples$group, col=colour,cex=0.8, main="Batch corrected MDS Plot")
        legend("topright", inset=c(-0.35,0), legend=c(unique(x$samples$group)), 
        col=colour, pch=15,box.lwd = 0,box.col = "white",bg = "white")
    dev.off()
}
